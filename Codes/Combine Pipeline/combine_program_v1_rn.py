# -*- coding: utf-8 -*-
"""Combine_Program_v1-RN.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1DLUfrt6jiLEQ55nAG4drtY37Sxta6Ay-
...
for train_idx, test_idx in cv_splits.split(data, groups=chassis_group):
  ...
  data_cat_train = categorical_pipeline(data_embed_in=data_t_embed, model_embed=model_embeddings, model_encoder=encoder_model_embed_85)
  # one hot encoding version of the categorical features
  '''
  # one hot encoding version of the categorical features
  data_one_hot = pd.get_dummies(data[cat_features], drop_first=True)
  data_one_hot['T_CHASSIS'] = data['T_CHASSIS']

  # split data to train and test - For the embedding informtion
  data_t_hot, data_v_hot = validationSplit(data_one_hot, split=0.2)
  data_t_hot.shape, data_v_hot.shape
  #data_cat_train = data_t_hot.drop(labels='T_CHASSIS', axis=1).values
  '''
   # merge together
  ...
  # fetch out the categorical representation
  data_cat_test = categorical_pipeline(data_embed_in=data_v_embed, model_embed=model_embeddings, model_encoder=encoder_model_embed_85)
  # one hot encoding version of the categorical features
  #data_cat_test = data_v_hot.drop(labels='T_CHASSIS', axis=1).values
  ...
  #rul_model.compile(optimizer='adam', loss='mae', metrics=['mae'])
  mae_score, ep_score = model_results(model_dev=regr)
